...
public class MaxCommand implements Command /*, Runnable */ {
	...
	public void execute() {
		...
		for (int idx = 1; idx < size; idx++) {
			if (IntStream.of(Arrays.stream(viewResult.getItems().get(result).getY()).mapToInt(Integer::parseInt).toArray()).sum() <
					IntStream.of(Arrays.stream(viewResult.getItems().get(idx).getY()).mapToInt(Integer::parseInt).toArray()).sum()) {
				result = idx;
			}
